...
class VFDDashboard(ctk.CTk):
    def __init__(self):
        ...
        # Window Setup
        self.title("Modbus Pro Dashboard - PdM Data Logger Edition")
        ...
        # State trackers for Thermal Resistance Trend (Delta t / Delta A)
        self.prev_temp = None
    ...
    def setup_main_area(self):
        ...
        # --- Top: Operations, Scanner, and PdM Tabs ---
        self.tabview = ctk.CTkTabview(self.main_frame, corner_radius=15)
        self.tabview.grid(row=0, column=0, sticky="new", pady=(0, 20))
        
        self.tab_pdm = self.tabview.add("PdM Data Logger")
        self.tab_ops = self.tabview.add("Manual Operations")
        self.tab_scan = self.tabview.add("Hacker Auto-Scanner")
        
        # ===== TAB 1: PDM DATA LOGGER =====
        self.tab_pdm.grid_columnconfigure((0, 1), weight=1)
        
        # Left Side: Inputs
        pdm_in_frame = ctk.CTkFrame(self.tab_pdm, fg_color="transparent")
        pdm_in_frame.grid(row=0, column=0, padx=10, pady=5, sticky="nsew")
        
        ctk.CTkLabel(pdm_in_frame, text="Manual Data Entry", font=ctk.CTkFont(size=18, weight="bold")).pack(pady=(0, 10))
        
        self.pdm_freq_var = ctk.StringVar(value="5.00")
        self.pdm_curr_var = ctk.StringVar(value="1.6")
        self.pdm_rpm_var = ctk.StringVar(value="150")
        self.pdm_volt_var = ctk.StringVar(value="319")
        self.pdm_temp_var = ctk.StringVar(value="58")
        inputs = [
            ("Frequency (Hz):", self.pdm_freq_var),
            ("Current (A):", self.pdm_curr_var),
            ("Speed (RPM):", self.pdm_rpm_var),
            ("Voltage (V):", self.pdm_volt_var),
            ("VFD_Temperature (°C):", self.pdm_temp_var),
        ]
        
        for label, var in inputs:
            row_frame = ctk.CTkFrame(pdm_in_frame, fg_color="transparent")
            row_frame.pack(fill="x", pady=2)
            ctk.CTkLabel(row_frame, text=label, width=130, anchor="w", font=ctk.CTkFont(weight="bold")).pack(side="left")
            ctk.CTkEntry(row_frame, textvariable=var).pack(side="left", fill="x", expand=True)

        # Right Side: Calculations
        pdm_calc_frame = ctk.CTkFrame(self.tab_pdm, fg_color="#1f2937", corner_radius=10)
        pdm_calc_frame.grid(row=0, column=1, padx=10, pady=5, sticky="nsew")
        
        ctk.CTkLabel(pdm_calc_frame, text="Advanced ML Features", font=ctk.CTkFont(size=18, weight="bold")).pack(pady=(10, 10))
        
        self.pdm_s_val = ctk.StringVar(value="-- VA")
        self.pdm_fric_val = ctk.StringVar(value="-- A/Hz")
        self.pdm_cool_val = ctk.StringVar(value="-- °C/VA")
        self.pdm_torque_val = ctk.StringVar(value="--")
        
        calcs = [
            ("Apparent Power (S):", self.pdm_s_val),
            ("Load Friction (A/Hz):", self.pdm_fric_val),
            ("Cooling Efficiency (°C/VA):", self.pdm_cool_val),
            ("Torque Estimate (Test):", self.pdm_torque_val),
        ]
        
        for label, var in calcs:
            row_frame = ctk.CTkFrame(pdm_calc_frame, fg_color="transparent")
            row_frame.pack(fill="x", pady=5, padx=10)
            ctk.CTkLabel(row_frame, text=label, width=160, anchor="w").pack(side="left")
            ctk.CTkLabel(row_frame, textvariable=var, font=ctk.CTkFont(weight="bold"), text_color="#1abc9c").pack(side="right")
            
        # Bottom: Save Button
        btn_save_pdm = ctk.CTkButton(self.tab_pdm, text="Calculate Features & Save to CSV", height=45, font=ctk.CTkFont(size=16, weight="bold"), fg_color="#e67e22", hover_color="#d35400", command=self.save_pdm_data)
        btn_save_pdm.grid(row=1, column=0, columnspan=2, padx=20, pady=10, sticky="ew")
    ...
    def save_pdm_data(self):
        try:
            f = float(self.pdm_freq_var.get())
            A = float(self.pdm_curr_var.get())
            r = float(self.pdm_rpm_var.get())
            U = float(self.pdm_volt_var.get())
            t = float(self.pdm_temp_var.get())
            
            # A. Apparent Power
            S = U * A
            
            # B. Load Friction Index (A/Hz)
            friction_idx = A / f if f > 0 else 0.0
                
            # C. Cooling Efficiency Index (C/VA)
            cooling_idx = t / S if S > 0 else 0.0
            
            # D. Torque Estimate (S / f)
            torque_est = S / f if f > 0 else 0.0
            
            # Update state for next calculation
            self.prev_temp = t
            self.prev_current = A
            
            # Update GUI Strings (Formatting logic)
            self.pdm_s_val.set(f"{S:.2f} VA")
            self.pdm_fric_val.set(f"{friction_idx:.4f} A/Hz")
            self.pdm_cool_val.set(f"{cooling_idx:.4f} C/VA")
            self.pdm_torque_val.set(f"{torque_est:.2f}")
            ...
            with open(file_path, "a", newline="") as csvfile:
                ...
                if not file_exists:
                    writer.writerow([
                        "Timestamp", "Frequency(Hz)", "Current(A)", "RPM", 
                        "Voltage(V)", "VFD_Temp(C)", "ApparentPower(VA)", 
                        "LoadFriction(A/Hz)", "CoolingEfficiency(C/VA)", "TorqueEst"
                    ])
                writer.writerow([
                    datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    f, A, r, U, t, 
                    round(S, 2), round(friction_idx, 4), round(cooling_idx, 4), round(torque_est, 2)
                ])
                
            self.log_text.insert(ctk.END, f"\n[PdM Logger] Analytics created & safely appended to 'pdm_motor_log.csv'!\n")
            self.log_text.see(ctk.END)

        except ValueError as e:
            messagebox.showerror("Input Error", "Please ensure all PdM inputs contain valid numbers.")
